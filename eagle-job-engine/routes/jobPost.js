...
router.post('/create',  function(req, res){
    ...
    newJob.save(function(error, result){
        if(error){
            ...
            req.flash('jobsuccess', '<p id="jobsuccess" class="bg-success" style="text-align: center; color: white;">Job posted successfully</p>');
            res.redirect(301, '/recruiter/id/' + result.userID + '/name/' + result.name + '#jobsuccess');
        };
    });
});
...
router.put('/update/', function(req, res){
    ...
    JobPostModel.findByIdAndUpdate(req.query._id, jobObj, {
        ...
        if(error){
            ...
            req.flash('updatesuccess', '<p id="updatesuccess" class="bg-success" style="text-align: center; color: white;">Job updated successfully</p>');
