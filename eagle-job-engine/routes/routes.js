...
`<form class="dropdown-item" action ="/auth/logout" method ="post"><button type="submit" class="btn btn-primary">Logout</button></form>`;
        ...
        JobPostModel.find({jobTitle : regex}).exec(function(error, result){
            if(error){
                ...
                res.render('homepage', {
                    ...
                    actions: "<a href='/login'><button class='btn btn-primary'>Login to     Apply</button></a>",
                    loggedoutmsg: req.flash('loggedoutmsg'),
                    buttons: buttons,
                    noMatch : noMatch,
                    isLoggedOut : "yes",
                    allJobs: "<a id='search-jobs-btn' href='/#allavailablejobs'><button class='btn btn-primary'>View All Jobs</button></a>"
                });
            };
        });
...
router.get('/recruiter/id/:_id/name/:name', routeProtect, function(req, res) {
    ...
    if(error){
        ...
        res.render('recruiter', {
            ...
            applicationData: result,
            actions: "<button class='receivedapps-btn btn btn-success' onclick='viewReceivedApps(this)'>Received Applications</button><button class='jobviewupdate-btn btn btn-primary delete-btn' onclick='viewJobPost(this)'>Update Details</button><button class='delete-btn btn btn-danger' onclick='deleteJobPost(this)'>Delete</button>",
        ...
        JobPostModel.find({jobTitle : regex}).exec(function(error, result){
            if(error){
                ...
                res.render('jobseeker', {
                    ...
                    applicationData: result,
                    actions: "<button onclick='viewJobPostAsApplicant(this)' class='jobviewapply-btn btn btn-primary'>View Details & Apply</button>",
                    topname: "<p class='display-4 col-8'>Logged in as: " + name + "</p>",
                    name: name,
                    ...
                    userID: applicantID,
                    noMatch : noMatch,
                    allJobs: "<a id='search-jobs-btn' href='/jobseeker/id/" + applicantID + "/name/" + name + "'><button class='btn btn-primary'>View All Jobs</button></a>"
                });
            };
        });
    ...
    JobPostModel.find({}).exec(function(error, result){
        if(error){
            ...
            res.render('jobseeker', {
                title: "Job Seeker Dashboard - Procruiter",
                applicationData: result,
                actions: "<button onclick='viewJobPostAsApplicant(this)' class='jobviewapply-btn btn btn-primary'>View & Apply</button>",
                topname: "<p class='display-4 col-8'>Logged in as: " + name + "</p>",
                name: name,
                accountType: "jobseeker",
                buttons: loggedIn,
...
router.get('/job/id/:_id/recid/:recid/name/:name', routeProtect, function(req, res) {
    ...
    JobPostModel.findById(jobID).exec(function(error, result){
        ...
        buttons: loggedIn,
        topname: "<p class='display-4 col-8'>Logged in as: " + req.params.name + "</p>",
        ID: jobID,
        ...
        if(error){
            ...
            UsersModel.findById(appID).exec(function(err, userData) {
                res.render('job-apply', {
                    ...
                    topname: "<p class='display-4 col-8'>Logged in as: " + userData.firstName + "</p>",
...
router.get('/appliedjob/id/:_id/appid/:appid', routeProtect, function(req, res) {
    let jobID = req.params._id;
    let appID = req.params.appid
    JobPostModel.findById(jobID).exec(function(error, result) {
        if(error) {
            ...
            JobApplicationModel.find({appID : appID}).exec(function(err, userData) {
                res.render('job-view', {
                    ...
                    userData: userData[0],
                    accountType: "jobseeker",
                    topname: "<p class='display-4 col-8'>Logged in as: " + userData[0].firstName + "</p>",
...
router.get('/received-applications/id/:_id/recid/:recid/name/:name', routeProtect, function(req, res) {
    let jobID = req.params._id;
    ...
    JobApplicationModel.find({jobID : jobID}).exec(function(error, result){
        ...
        name: req.params.name,
        topname: "<p class='display-4 col-8'>Logged in as: " + req.params.name + "</p>",
        actions: "<button class='btn btn-success' onclick='shortlistApplication(this)'>Shortlist</button><button class='delete-btn btn btn-danger' onclick='deleteApplication(this)'>Delete</button>",
...
router.get('/reset/:token', function(req, res) {
    UsersModel.findOne({ resetPasswordToken: req.params.token, resetPasswordExpires: { $gt: Date.now() } }, function(err, user) {
      if (!user) {
        req.flash('invalidToken', '<p class="bg-danger" style="text-align: center; color: white;">Password reset token is invalid or expired.</p>');
