...
router.post('/id/:_id/recid/:recid', function(req, res){
    JobApplicationModel.findById(req.params._id).exec(function(error, applicantsData){
    ...
    newShortlist.save(function(error , result){
        if(error){
            ...
            req.flash('usershortlisted', '<p id="usershortlisted" class="bg-success" style="text-align: center; color: white; margin-top: 20px;">Applicant shortlisted successfully and saved in your profile</p>');
...
router.delete('/delete/:_id', function(req, res){
    ShortlistedModel.findByIdAndDelete(req.params._id).exec(function(error, result){
        if(error){
            throw error;
        } else {
